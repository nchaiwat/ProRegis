...
let BackofficeService = BackofficeService_1 = class BackofficeService {
    logRepository;
    ...
    productsService;
    configService;
    logger = new common_1.Logger(BackofficeService_1.name);
    constructor(logRepository, productionOrderRepository, registrationRepository, systemSettingRepository, auditLogRepository, telegramService, sapService, productsService, configService) {
    ...
    async getSystemSettings() {
        ...
        const now = new Date();
        if (!result['QR_CODE_MODE'])
            result['QR_CODE_MODE'] = { value: 'STATIC', updatedAt: now };
        if (!result['VERIFICATION_MODE'])
            result['VERIFICATION_MODE'] = { value: 'OTP', updatedAt: now };
        if (!result['EMAIL_OTP_MODE'])
            result['EMAIL_OTP_MODE'] = { value: 'TEST', updatedAt: now };
        if (!result['SMTP_HOST'])
            result['SMTP_HOST'] = { value: 'smtp.gmail.com', updatedAt: now };
        if (!result['SMTP_PORT'])
            result['SMTP_PORT'] = { value: '587', updatedAt: now };
        if (!result['SMTP_SECURE'])
            result['SMTP_SECURE'] = { value: 'false', updatedAt: now };
        if (!result['SMTP_USER'])
            result['SMTP_USER'] = { value: 'itwindowasia@gmail.com', updatedAt: now };
        if (!result['SMTP_PASS'])
            result['SMTP_PASS'] = { value: '', updatedAt: now };
        if (!result['SMTP_FROM_NAME'])
            result['SMTP_FROM_NAME'] = { value: 'Window Asia Warranty', updatedAt: now };
        if (!result['SMTP_FROM_EMAIL'])
            result['SMTP_FROM_EMAIL'] = { value: 'itwindowasia@gmail.com', updatedAt: now };
        return result;
    }
    ...
    async generateBatch(actor, docNum, startSeq, quantity, ipAddress, previewOnly = false) {
        ...
        const bypassValidation = this.configService.get('SAP_BYPASS_VALIDATION', 'false') === 'true';
        let poInfo = null;
        try {
        ...
        if (!poInfo) {
            if (bypassValidation) {
                ...
                throw new common_1.BadRequestException(`ไม่พบเลขที่สั่งผลิต (PD) ${docNum} นี้ในระบบ SAP B1`);
            }
        }
        ...
        if (!itemCodeUpper.startsWith('FA') && !itemCodeUpper.startsWith('FU')) {
            if (bypassValidation) {
                ...
                throw new common_1.BadRequestException(`รหัสสินค้า (Item Code) ของใบสั่งผลิตนี้คือ ${activePoInfo.itemCode} ซึ่งต้องขึ้นต้นด้วย FA หรือ FU เท่านั้น`);
            }
        }
        if (requestedEndSeq > activePoInfo.plannedQty) {
            if (bypassValidation) {
                ...
                throw new common_1.BadRequestException(`จำนวน QR ที่ต้องการสร้าง เกินกว่าจำนวนที่ระบุในแผนการผลิตของ SAP B1 (แผนระบุสูงสุดได้คือ ${activePoInfo.plannedQty} ชิ้น, แต่ครั้งนี้ขอรันถึงลำดับที่ ${requestedEndSeq})`);
            }
        }
            ...
            if (prodDateStr) {
                ...
                if (!isNaN(prodDate.getTime())) {
                    ...
                    if (diffDays <= 30)
                        ...
                        lagOver180++;
                    const monthKey = prodDate.toISOString().substring(0, 7);
    ...
    async getProductionTrackerList(mode) {
        ...
        trackerList.sort((a, b) => new Date(b.latestRequestDate).getTime() - new Date(a.latestRequestDate).getTime());
        return trackerList;
    }
    async checkProduct(token, label, registrationId) {
        let docNum = null;
        let seqNum = null;
        ...
        if (registrationId) {
            ...
            if (token) {
                ...
                if (cleanLabel.length === 12 && /^\d+$/.test(cleanLabel)) {
                    ...
                    throw new common_1.BadRequestException('รหัสสินค้าต้องเป็นตัวเลข 9 หลัก หรือ 12 หลัก');
                }
            }
            else {
            ...
            registration = await this.registrationRepository.findOne({
                where: {
                    docNum: docNum || '',
                    seqNum: seqNum ? seqNum : (0, typeorm_2.IsNull)()
                },
        ...
        if (docNum) {
            try {
                ...
                throw new common_1.BadRequestException(`ข้อผิดพลาดจาก SAP B1: ${detailMsg}`);
            }
        }
    ...
    async getCustomImages() {
        ...
        for (const meta of metas) {
            ...
            const qb = this.registrationRepository
                ...
                .innerJoin(production_order_entity_1.ProductionOrder, 'po', 'po.docNum = reg.docNum')
    ...
    async clearTestData(tables) {
        ...
        await this.registrationRepository.manager.query(`TRUNCATE TABLE ${tableList} RESTART IDENTITY CASCADE;`);
    }
};
exports.BackofficeService = BackofficeService;
exports.BackofficeService = BackofficeService = BackofficeService_1 = __decorate([
    ...
    __param(7, (0, common_1.Inject)((0, common_1.forwardRef)(() => products_service_1.ProductsService))),
