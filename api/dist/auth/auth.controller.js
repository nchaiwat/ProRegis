...
let AuthController = class AuthController {
    ...
    async login(body, req) {
        ...
        if (!user) {
            await this.auditService.logAction(usernameNormalized, 'LOGIN_FAILED', 'Auth', null, ipAddress, userAgent, { reason: 'Username not found' });
        ...
        if (!isPasswordCorrect) {
            ...
            await this.auditService.logAction(user.username, 'LOGIN_FAILED', 'Auth', user.id, ipAddress, userAgent, { reason: 'Incorrect password', failedAttempts: user.failedAttempts });
            throw new common_1.UnauthorizedException('ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง');
        }
        ...
        await this.auditService.logAction(user.username, 'LOGIN_SUCCESS', 'Auth', user.id, ipAddress, userAgent);
