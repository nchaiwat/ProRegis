...
export default function GeneratePage() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const filename = `QR_Batch_${docNum}_seq${String(finalSeq).padStart(3, "0")}_qty${finalQty}.csv`;
      a.href = url;
      a.download = filename;
      ...
      setSuccessMsg(`✅ ดาวน์โหลดไฟล์ "${filename}" สำเร็จ — บันทึก Log และส่งแจ้งเตือนเข้า Telegram Group เรียบร้อยแล้ว`);
      setPreview([]);
    } catch (err) {
      setError(err instanceof Error ? err.message : "เกิดข้อผิดพลาด");
    } finally {
    ...
    <div className="max-w-2xl mx-auto space-y-8 animate-success">
      <div className="mb-6">
        ...
        <p className="text-sm text-on-surface-variant mt-1">
          ระบบจะ Generate ไฟล์ CSV (UTF-8 + BOM) พร้อม Token เข้ารหัสแบบ <b>AES-128-CBC</b> สำหรับเครื่องยิง QR
        </p>
      </div>
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-6 shadow-sm space-y-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setStartSeq(val === "" ? "" : parseInt(val, 10));
                setError("");
                setSuccessMsg("");
                setPreview([]);
              }}
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              ...
              <div>
                ...
                <p className="font-mono font-bold text-sm text-primary">
                  {String(startSeq || 1).padStart(3, "0")} → {String((startSeq || 1) + (quantity || 1) - 1).padStart(3, "0")}
                </p>
              </div>
              <div>
                <p className="text-[10px] text-outline uppercase">PD แรก</p>
                <p className="font-mono font-bold text-sm text-primary">
                  {docNum}{String(startSeq || 1).padStart(3, "0")}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex gap-3 pt-2">
          <button
            ...
            className="flex-1 h-12 border-2 border-secondary text-secondary font-bold rounded-xl hover:bg-secondary/5 active:scale-[0.98] transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
          ...
          <button
            ...
            className="flex-1 h-12 bg-secondary text-white font-bold rounded-xl shadow-md hover:opacity-95 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
          >
            {isGenerating ? (
        ...
        <div className="mt-6 bg-surface-container-lowest border border-outline-variant rounded-2xl overflow-hidden shadow-sm">
          <div className="px-5 py-4 border-b border-outline-variant flex items-center gap-2">
            ...
            <h3 className="font-bold text-sm text-primary">
              Preview — {preview.length} รายการ (Column Code จะเข้ารหัสด้วย AES-128-CBC เมื่อ Download)
            </h3>
          </div>
              ...
              <tbody className="divide-y divide-outline-variant/50">
                {preview.slice(0, 10).map((row, i) => (
                  <tr key={i} className="hover:bg-surface-container-low/50 transition-colors">
                    <td className="px-4 py-2.5 text-outline font-mono">{(startSeq || 1) + i}</td>
